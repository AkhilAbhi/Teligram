...
def uploadMovie(msgId,chId,name,title,b):
...
def uploadData(Client,message,name,url,chanel):
  print("uplodinggg")
...
def movieSerc(Client,message,qury):
  ...
  for rs in resul:
    ...
    message.reply_photo("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRzeZtCv67U_CkfkEb602B0Kpzw47rnl2Tjnw&usqp=CAU",caption="Thear is your result ",reply_markup=bt) 
  print(i)
  if i==0:
    message.reply_text("സ്പെല്ലിങ് കറക്റ്റ് ആണോ എന്ന് ചെക്ക് ചെയുക \nഅല്ലെങ്കിൽ ആ മൂവി ഞങ്ങടെ  കയിൽ ഇല്ല ഉടനെ  അപ്‌ലോഡ് ചെയ്യുന്നേആയിരിക്കുംം ")
    logC(Client,message)
  else:
    pass
...
def uplodedData(Client,message):
  ...
  e = c+d
  idd = int(e)
  uploadMovie(messageId,chanelId,movieName,title,a)
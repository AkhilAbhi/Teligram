...
def test(Client,message):
  ...
  if i > 0:
    message.reply_text(f"Here is what i found for your query ({message.text})", reply_markup=inline_keyboard)
  else:
    message.reply_text(f"sorry i can't find any movie!! ({message.text})\nCheck if the spelling is correct")
    logC(Client,message)

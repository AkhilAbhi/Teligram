...
def chekking(Client,message):
  text=message.text
  if text=="!up":
    updata(Client,message)
  else:
    pass
...
def updata(Client,message):
  ...
  if chekWich=="›":
    ...
    message.reply_text("അതുപോലെ അദ്യം #ഇട്ടട്ടു മിവിയുടെ name കൊടുക്ക്")
  elif chekWich == "#":
    global Name
    tempN = message.text
    Name = tempN.replace("#","")
    message.reply_text("ലാസ്റ്റ് aa ഫയലിന്റ്റ് url ₹ കൊടുക്ക്")
  elif chekWich == "₹":
    global url
    tempu = message.text
    url = tempu.replace("₹","")
    message.reply_text("അവസനും ഡാറ്റബാസിലോട്ട് അപ്‌ലോഡ് ചെയ്യാൻ + എന്ന് കൊടുക്ക്‌")
  elif chekWich == "+":
    if len(Name) > 3 and len(chanel) > 3 and len(url) > 3:
      uploadData(Client,message,Name,url,chanel)
    else:
      message.reply_text("Niggale Endhokkeyo miss chythattunde !helpe ennu send chyeu")
    #print(Name)
    #print(chanel)
    #print(url)
